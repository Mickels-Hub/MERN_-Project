...
export default function Search() {
  ...
  return (
    <div className="flex flex-col md:flex-row bg-[#030712] min-h-screen text-slate-100">
      {/* Sidebar Filters */}
      <div className="p-7 border-b md:border-r border-slate-800 md:min-h-screen w-full md:w-[350px] shrink-0">
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <select
              ...
              id="sort_order"
              className="bg-slate-900 border border-slate-800 rounded-xl p-3 w-full focus:outline-none text-white text-sm"
            >
          ...
          <button className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white p-3.5 rounded-xl uppercase font-bold transition shadow-lg text-sm cursor-pointer mt-2">
            Apply Filters
          </button>
        </form>
      </div>
        ...
        <div className="p-4 sm:p-7 flex flex-wrap gap-6 justify-center sm:justify-start">
          ...
          {!loading && listings && listings.map((listing) => (
            <Link
              ...
              key={listing._id}
              className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl group hover:border-slate-700 transition duration-300 w-full sm:w-[300px] flex flex-col"
            >
              <img
              ...
              <div className="p-5 space-y-2 flex-1 flex flex-col justify-between">
                ...
                <p className="text-indigo-400 font-extrabold text-base pt-2">₦{listing.regularPrice?.toLocaleString()}</p>
              </div>
            </Link>
          ))}
        </div>
